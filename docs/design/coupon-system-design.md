...
- **品类券 ≠ 店铺整单券，二者跨层叠加**：品类券作用于「指定品类小计」(商品层 layer 0)、店铺整单满减作用于「整单」(店铺层 layer 1)，分属不同层 → **可叠加**(实测见 `test_a_stacking`)。这里的"店铺整单券"指**商家自有的店铺级**满减/折扣(`shop_full_reduce`/`shop_discount`，绑定单个 `shop_id`)；**平台级**整单满减是另一类型 `platform_full_reduce`(神券，layer 2)。互斥只发生在**同层**(店铺满减 vs 店铺折扣、神券 vs 平台立减)。
...
**🟡 非阻塞可调项（已给保守默认）**：神券与会员红包是否同叠（默认可叠，张力最大）、
达标线（默认 rating 4.2，可改 `--rating-floor`）。

> 📌 **更正(回应 PR review)**：早期本节曾把"品类券与店铺整单券互斥（默认互斥）"列为可调项，**实为笔误**。
> 品类券属商品层(或平台级品类活动)、店铺整单满减属店铺级，分属不同层、**应当叠加**；实现里二者本就在不同
> `exclusive_group`、跨层生效(见 §4.1，`test_a_stacking` 实测同一单同时命中 ce-001 店铺满减 + ce-003 品类券)。互斥仅限同层。
...
## 11. 采购侧对标（小象超市等日用品，复用同一引擎）

Skill 2 同时覆盖采购(猫粮/纸/水…)。采购**复用同一套结构化券引擎**，但券设计与外卖有别(见决策时讨论)，故补了两个引擎能力 + 一类数据，**全部限美团平台内**(小象超市/物美/便利店等美团商家，不跨盒马/叮咚)。

### 11.1 采购券与外卖券的关键不同 → 引擎/数据应对
| 不同 | 应对 |
|---|---|
| 「第二件半价/8折」是采购主力 | 引擎加 `discount.mode="nth"`(`nth`/`nth_rate_bps`；每 nth 件 1 件打折，按 qty 触发) |
| 规格/单价维度(大包装+满减常**券后单价**更低) | Item 加 `unit_qty`/`unit`；`optimize` 加 **`unit` 目标**(按券后单价=payable/总规格量 排序)。采购"最优解"= **满足周期需求量前提下、券后单价最优** |
| 凑单为「免运费/起送 + 反正要买的」 | 复用满减/`delivery_fee`(满39包邮)+ 凑单；意图层把周期到点的几样凑一单 |
| 满赠/加价购 | v1 不进优化器(只文案)，避免多目标污染 |
| 时间维度是「天级」(会员日/大促) | 复用状态机(periodic/event)，窗口换成天/星期几 |

### 11.2 数据 & 接线
- 采购 SKU 用 **`gsku-*`** 前缀(多规格、绑美团采购商家)，`load_catalog` 同时收 `dish-*`+`gsku-*`；但 `MealContext.dishes`(外卖 recommend 候选池)只收 `dish-*`，**采购 SKU 不污染外卖**。
- `coupon_engine.json` 加采购券：满99减20、抽纸第二件8折(`nth`)、满39包邮、物美满减、便利店新客。
- `cmd_grocery` 重写：周期触发(`grocery_due`) → 每需求在多商家/多规格/qty 候选里 `optimize(objective="unit")` 比券后单价 → 再给"凑小象超市一单"(满99减20+包邮)方案。
- 单测：`test_y`(规格单价·5kg大包装单价最优)、`test_z`(nth·第二件8折按 qty 触发)、`test_aa`(购物车凑满减)。
