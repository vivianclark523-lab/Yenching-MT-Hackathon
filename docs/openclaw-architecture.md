...
| 配置管家人设/记忆 | **已确认**：稳定画像走 OpenClaw 原生 `~/.openclaw/workspace/USER.md`，动态偏好走 `~/.openclaw/workspace/MEMORY.md`；仓库镜像在 `openclaw-workspace/`。不要再用 `preferences.md` / `habits.md` / `social.md` 三文件。 |
...
- [x] **管家人设 / 记忆** → 稳定画像走 `USER.md`，动态偏好走 `MEMORY.md`，运行时位置 `~/.openclaw/workspace/`，仓库镜像 `openclaw-workspace/`。忌口/预算/常驻地/同行人这类硬约束写进 `USER.md`。
- [ ] **用户偏好持久化** → Demo 先用原生 `USER.md` / `MEMORY.md` 手动维护；是否开启 dreaming 自动沉淀后续再定。
