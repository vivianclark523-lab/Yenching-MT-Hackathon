...
"现在几点"由**虚拟时钟**决定，不是真实世界时间、也不是消息时间戳。**任何"是否营业 / 现在开没开 / 来不来得及 / 几点 / 今晚 / 饭点到没到 / 还有多久关门"的判断，一律以脚本输出的 `virtual_now` 字段为准**（`business_context` / `queue_context` 都会返回它）；**严禁**用你自己感知的系统时间。高德返回的营业时间只是营业时段，不代表此刻开着。
