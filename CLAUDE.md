...
OpenClaw 原生自动注入 `USER.md`（稳定用户画像）和 `MEMORY.md`（动态偏好沉淀）。忌口、预算、常驻地、饭点习惯、同行人及其忌口统一写进 `~/.openclaw/workspace/USER.md`；订单/路线反馈等变化中的偏好写进 `~/.openclaw/workspace/MEMORY.md`。仓库内镜像放在 `openclaw-workspace/`。Skill 2/3 **只消费这两份原生画像，不再维护或读取** `preferences.md` / `habits.md` / `social.md` / `profile.py` 这类自定义三文件机制。
