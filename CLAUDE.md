...
OpenClaw 的记忆是**工作区(`~/.openclaw/workspace/`)里的 markdown 文件**:`USER.md`（用户画像:忌口/预算/习惯/同行人）+ `MEMORY.md`（长期精炼)**每次会话自动注入上下文**;`memory/YYYY-MM-DD.md`（每日笔记)今/昨自动加载、更早的靠 `memory_search` 召回。**Skill 代码不需要主动读** —— 画像随 USER.md/MEMORY.md 自动进上下文,全项目(Skill 1/2/3)共享同一份。

> ⚠️ 早期本文档曾写"`~/.openclaw/agents/main/memory/` 下的 `preferences.md`/`habits.md`/`social.md`"——**那是误记**:OpenClaw 不会自动加载这类自定义文件,统一走原生 `USER.md`/`MEMORY.md`。决策与迁移见 [`docs/design/memory-architecture-alignment.md`](docs/design/memory-architecture-alignment.md)。
