...
│   ├── imagegen.py                    ← 文生图调用（**唯一入口**，含 fallback）
...
| 调豆包文生图 | `from scripts.imagegen import generate_card` | ❌ 直接调豆包 SDK；自带 fallback 到文字版 |
| 读虚拟时间 | `from mocks.clock import virtual_now` | ❌ `datetime.now()` |
| 读用户偏好 / 习惯 | 不需要主动读——OpenClaw memory-core 自动注入到 LLM 上下文 | ❌ 自己写代码去读 `~/.openclaw/agents/main/memory/` |
...
1. **不用 Canvas（macOS-only），用文生图 API**（已有 `scripts/imagegen.py`）
